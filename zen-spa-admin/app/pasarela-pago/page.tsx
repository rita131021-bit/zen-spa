...
export default function PasarelaPagoPage() {
  return (
    <AdminShell aside={<PaymentAside />}>
      <PageHeader eyebrow="pay" title="Pasarela de Pago" subtitle="Gestiona los pagos y cobros de tus clientes de forma simple y segura." action={<button className="outline-button">Configuracion de pagos</button>} />
      ...
      <div className="tab-strip"><button className="active">Pagos</button><button>Metodos de pago</button><button>Historial de pagos</button><button>Configuracion</button></div>
      <section className="payment-layout">
        ...
        <article className="panel-card form-grid"><h3>Informacion del pago</h3><label>Estado del pago<select><option>Pendiente</option></select></label><label>Monto total<input value="$ 18.500" readOnly /></label><label>Monto pagado<input value="$ 0" readOnly /></label><label>Notas<textarea placeholder="Agrega una nota sobre el pago..." /></label><button className="new-turn-button">Registrar pago</button></article>
